...
def prepare_presentation_slide(
        slide, 
        slide_title, 
        video_with_contours_OD_path, 
        video_with_contours_OS_path, 
        thumbnail_with_contours_OD_path, 
        thumbnail_with_contours_OS_path, 
        plot_path_OD,
        plot_path_OS, 
        config
        ):
    ...
    if os.path.exists(video_with_contours_OD_path):
        slide.shapes.add_movie(video_with_contours_OD_path, left, top, width=video_width, height=video_height, poster_frame_image=thumbnail_with_contours_OD_path)

    # Add video with contours
    if os.path.exists(video_with_contours_OS_path):
        slide.shapes.add_movie(video_with_contours_OS_path, left, top + Inches(2.5) + Inches(0.4), width=video_width, height=video_height, poster_frame_image=thumbnail_with_contours_OS_path)

    # Add growth curves - OD
    plot_width = Inches(4.91/1.5)
    plot_height = Inches(5.67/1.5)
    # bypass plot path with gompertz
    mrn, lat, _ = os.path.basename(plot_path_OD).split('_')
    ...
    if os.path.exists(plot_path_OD):
        slide.shapes.add_picture(plot_path, Inches(3.0), Inches(2.0), width=plot_width, height=plot_height)

    # Add growth curves - OS
    plot_width = Inches(4.91/1.5)
    plot_height = Inches(5.67/1.5)
    # bypass plot path with gompertz
    mrn, lat, _ = os.path.basename(plot_path_OS).split('_')
    if config.gompertz is not None:
        if os.path.exists(os.path.join(config.results_folder, config.gompertz, f'gompertz_plots/gompertz-fit-{mrn}_{lat}.png')):
            plot_path = os.path.join(config.results_folder, config.gompertz, f'gompertz_plots/gompertz-fit-{mrn}_{lat}.png')

    if os.path.exists(plot_path_OS):
        slide.shapes.add_picture(plot_path, Inches(3.0) + plot_width + Inches(1.0), Inches(2.0), width=plot_width, height=plot_height)
...
def main():
    ...
    for result in tqdm(results, desc="Creating slides", unit="slide"):
        ...
        prepare_presentation_slide(
            slide,
            f'MRN: {result["mrn_mapped"]} | Age(v1): {result["age_v1"]} | PRS: {result["pr_score"]} | In registry?: {result["in_registry"]}',
            os.path.join(save_to, 'metadata', 'videos_wcontours', f'{result["mrn"]}_OD_wcontours.mp4'),
            os.path.join(save_to, 'metadata', 'videos_wcontours', f'{result["mrn"]}_OS_wcontours.mp4'),
            os.path.join(save_to, 'metadata', 'thumbnails_wcontours', f'{result["mrn"]}_OD_wcontours.png'),
            os.path.join(save_to, 'metadata', 'thumbnails_wcontours', f'{result["mrn"]}_OS_wcontours.png'),
            os.path.join(save_to, 'metadata', 'plots', f'{result["mrn"]}_OD_plot.png'),
            os.path.join(save_to, 'metadata', 'plots', f'{result["mrn"]}_OS_plot.png'),
            args
        )

        done.append(result['mrn'])
